<?php include ("topnav.php")?>
<!--start-detalis-->
<div class="main-backgrond">
  <div class="radous">
    <div class="bacground-details text-center">
      <div class="row">
        <div class="col-xl-4  col-lg col-md-12">
          <div class="imge-details d-xl-none">
            <img src="../project-myserves/layot/img/pexels-anamul-rezwan-1216544 (1).jpg" alt="">
          </div>
        </div>
        <div class="col-xl-4  col-lg col-md-12">
          <div class="text-detais">
            <h3>محمد ابو غزاله</h3>
            <i class="fas fa-briefcase"></i><span>خدمات قانونية</span>
          </div>
        </div>
        <div class="col-xl-4  col-lg col-md col-xs">
          <div class="mar-btn-detals">
            <a href="#"><button type="button" class="btn  btn-details">تواصل مع البائع</button></a><span><a href="#"><button type="button" class="btn btn-secondary btn-details-tow"><i class="fas fa-shopping-cart icon-cart"></i><span>طلب الخدمة</span></button></a></span>
          </div>
          <div class="ul-details-tow-ico">
            <ul>
              <li><i class="fas fa-truck"></i><span>10 طلبات جاري تنفيذها</span></li>
              <li> <i class="fas fa-reply"></i><span>م. سرعة رد: 10 ساعات</span></li>
              <li><i class="fas fa-shopping-cart"></i><span>10 اشتروا هذه الخدمة</span></li>
            </ul>
          </div>
        </div>
      </div>
    </div>
    <div class="back-ground-par">
      <div class="imge-details-tow d-none d-xl-block">
        <img src="../project-myserves/layot/img/pexels-anamul-rezwan-1216544 (1).jpg" alt="">
      </div>
      <div class="row">
        <div class="container">
          <div class="col-lg-12">
            <div class="mr-details text-center">
              <h1 class="h1-detals">تفاصيل الخدمه:</h1>
              <p class="prg-dtails">
                سأقوم بتدقيق ومراجعة كافة مخططات المنزل او الفيلا او المشروع الهندسي الخاص بك .
                التدقيق يشمل المخططات المعمارية والانشائية والالكتروميكانيك , او حسب الطلب تحديدا .
                الخدمة تشمل الاشارة الى نواقص المخططات او المشاكل بها ان وجدت مع امكانية التعديل وتقديم ما يلزم من استكمال للمخططات الموجودة سواء معماريا او انشائيا او توزيع انارة وفرش وتكييف وغيرها .
                في حال كمالية المخططات استطيع تقديم افكار ومساعدة لتطويرها وفق ما اراه مناسبا من خبرتي في هذا المجال .
              </p>
            </div>
          </div>
          <hr class="hr-details">
          <div class="col-lg-12">
            <div class="back-ground-comment-part-one">
              <form>
                <div class="form-group">
                  <label class="lable-details" for="exampleFormControlTextarea1"><span><i style="color:#e59560" class="fas fa-comment-alt "></i>&nbsp;</span>آراء المشترين</label>
                  <textarea class="form-control" id="exampleFormControlTextarea1" rows="2"></textarea><span><button type="submit" class="btn btn-textarea"> <span style="margin-left:5px"><i class="fas fa-comments"></i></span>تعليق</button>
                </div>
              </form>
            </div>
          </div>
              <div class="col-lg-12">
                <div class="back-ground-comment-part-one">
                  <img class="imge-comment" src="../project-myserves/layot/img/pexels-anamul-rezwan-1216544 (1).jpg" alt=""><span class="commnt-span">حمزه ابو قويدر</span><i class="far fa-clock icon-commint-details"></i><span class="span-commnt-data">منذ 6 أشهر و8 أيام</span>
                  <p class="commnt-text">خدمة احترافية جدا وتعامل راقي .</p>
                  <i class="fas fa-reply icon-commint-details-tow"></i><span class="replay-commnt">0ردود</span><small class="replay-feedback" style="margin-right:20px">رد على تعليق</small>
                </div>
              </div>
              <div class="col-lg-12">
                <div class="back-ground-comment-part-tow">
                  <img class="imge-comment" src="../project-myserves/layot/img/pexels-anamul-rezwan-1216544 (1).jpg" alt=""><span class="commnt-span">حمزه ابو قويدر</span><i class="far fa-clock icon-commint-details"></i><span class="span-commnt-data">منذ 6 أشهر و8 أيام</span>
                  <p class="commnt-text">خدمة احترافية جدا وتعامل راقي .</p>
                </div>
              </div>
              <div class="col-lg-12">
                <div class="replay-comment-details">
                  <form action="#">
                    <div class="form-group">
                      <textarea class="form-control" id="exampleFormControlTextarea1" rows="2"></textarea><span><button type="submit" class="btn btn-textarea"> <span style="margin-left:5px"><i class="fas fa-comments"></i></span>رد</button>  
                    </div>
                  </form>
                </div>
              </div>
              <div class="col-lg-12">
                <div class="back-ground-comment-part-one">
                  <img class="imge-comment" src="../project-myserves/layot/img/pexels-anamul-rezwan-1216544 (1).jpg" alt=""><span class="commnt-span">حمزه ابو قويدر</span><i class="far fa-clock icon-commint-details"></i><span class="span-commnt-data">منذ 6 أشهر و8 أيام</span>
                  <p class="commnt-text">خدمة احترافية جدا وتعامل راقي .</p>
                  <i class="fas fa-reply icon-commint-details-tow"></i><span class="replay-commnt">0رد</span><small class="replay-feedback" style="margin-right:20px">رد على تعليق</small>
                </div>
              </div>
              <div class="col-lg-12">
                <div class="back-ground-comment-part-tow">
                  <img class="imge-comment" src="../project-myserves/layot/img/pexels-anamul-rezwan-1216544 (1).jpg" alt=""><span class="commnt-span">حمزه ابو قويدر</span><i class="far fa-clock icon-commint-details"></i><span class="span-commnt-data">منذ 6 أشهر و8 أيام</span>
                  <p class="commnt-text">خدمة احترافية جدا وتعامل راقي .</p>
                </div>
              </div>
              <div class="col-lg-12">
                <div class="replay-comment-details">
                  <form action="">
                    <div class="form-group">
                      <textarea class="form-control" id="exampleFormControlTextarea1" rows="2"></textarea><span><button type="submit" class="btn btn-textarea"> <span style="margin-left:5px"><i class="fas fa-comments"></i></span>رد</button>
                    </div>
                  </form>
                </div>
              </div>
              <div class="col-lg-12">
                <div class="back-ground-comment-part-one">
                  <img class="imge-comment" src="../project-myserves/layot/img/pexels-anamul-rezwan-1216544 (1).jpg" alt=""><span class="commnt-span">حمزه ابو قويدر</span><i class="far fa-clock icon-commint-details"></i><span class="span-commnt-data">منذ 6 أشهر و8 أيام</span>
                  <p class="commnt-text">خدمة احترافية جدا وتعامل راقي .</p>
                  <i class="fas fa-reply icon-commint-details-tow"></i><span class="replay-commnt">0رد</span><small class="replay-feedback" style="margin-right:20px">رد على تعليق</small>
                </div>
              </div>
              <div class="col-lg-12">
                <div class="back-ground-comment-part-tow">
                  <img class="imge-comment" src="../project-myserves/layot/img/pexels-anamul-rezwan-1216544 (1).jpg" alt=""><span class="commnt-span">حمزه ابو قويدر</span><i class="far fa-clock icon-commint-details"></i><span class="span-commnt-data">منذ 6 أشهر و8 أيام</span>
                  <p class="commnt-text">خدمة احترافية جدا وتعامل راقي .</p>
                </div>
              </div>
              <div class="col-lg-12">
                <div class="replay-comment-details">
                  <form action="">
                    <div class="form-group">
                      <textarea class="form-control" id="exampleFormControlTextarea1" rows="2"></textarea><span><button type="submit" class="btn btn-textarea"> <span style="margin-left:5px"><i class="fas fa-comments"></i></span>رد</button>
                    </div>
                  </form>
                </div>
              </div>
          </div>      
      </div>
    </div>
  </div>
</div>
<!--end-details-->
<?php include('loding.php');?>

<?php include ("footer.php");?>






